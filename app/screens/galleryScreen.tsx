...
const GalleryScreen = () => {
  ...
  const takePicture = async () => {
    if (cameraRef.current) {
      ...
      const token = await asyncStorageService.get<string>(asyncStorageService.KEYS.userToken);
      if (token) {
        await apiService.saveImage(token, photo.base64, photo.width, photo.height);
        setImages([...images, { encodedData: photo.base64 }]);
      }
  ...
  return (
    <View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => setSelectedImage(item.encodedData)}>
              <Image source={{ uri: `data:image/png;base64,${item.encodedData}` }} style={{ width: 100, height: 100 }} />
            </Pressable>
          )}
        />
      )}
      ...
      <Modal visible={!!selectedImage} transparent={true}>
        <Pressable style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.8)", justifyContent: "center", alignItems: "center" }} onPress={() => setSelectedImage(null)}>
          {selectedImage && <Image source={{ uri: `data:image/png;base64,${selectedImage}` }} style={{ width: 300, height: 300 }} />}
        </Pressable>
      </Modal>
    </View>
  );
